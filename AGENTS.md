...
### Decision lifecycle
Use `valis_lifecycle` to manage decision status:

- **Deprecate**: When a decision is no longer valid, call `valis_lifecycle` with `action: "deprecate"`, `decision_id`, and a `reason`. This marks it deprecated and flags any decisions that depend on it.
- **Promote**: When a proposed decision is confirmed, call `valis_lifecycle` with `action: "promote"` to transition it from proposed to active.
- **History**: Call `valis_lifecycle` with `action: "history"` to view the full status change timeline of a decision.
- **Supersede via replaces**: When storing a new decision that replaces an old one, pass `replaces: "<old-decision-uuid>"` to `valis_store`. The old decision is automatically marked as superseded.
...
When `valis_store` returns a `contradictions` array, it means the newly stored decision conflicts with existing active decisions in overlapping areas. When you see contradiction warnings:
...
When you receive a `<channel source="valis" event="decision_deprecated">`, check if the deprecated decision affects your current work and adjust accordingly.

When you receive a `<channel source="valis" event="new_decision">` with `origin: "remote"`, a teammate stored a new decision — consider if it affects your current task.

When you receive a `<channel source="valis" event="contradiction_detected">`, two active decisions may conflict — review and resolve if needed.
<!-- valis:end -->
