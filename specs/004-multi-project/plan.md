...
Add multi-project support to Valis so that organizations can maintain
...
`~/.valis/config.json` for org credentials + per-directory
`.valis.json` for project identity) enables automatic project
...
**Config**: Two-tier — global `~/.valis/config.json` (org creds, unchanged) + per-directory `.valis.json` (project_id, project_name). Walk-up resolution from cwd.
...
| II | Minimally Invasive | PASS | Same MCP + channel integration. No new IDE interception. `.valis.json` is a passive config file read on demand. |
| III | Non-Blocking | PASS | Missing `.valis.json` degrades to "no project" message, not an error. Missing project_id in Qdrant falls back to org-level search. Realtime failure is silent. |
...
| VI | Auto-Capture by Default | PASS | Unchanged. Auto-capture layers now include `project_id` from resolved config. File watcher and stop hook read `.valis.json` automatically. |
...
**Security & Data Integrity**: Project-scoped RLS via `effective_project_id()` function. JWT includes `project_id` claim. Qdrant queries filter by `project_id`. Cross-project search uses org-level JWT (no project_id) with application-level access control via `project_members`. Per-directory `.valis.json` contains no secrets.
...
│   └── project.ts              # NEW: per-directory .valis.json read/write/walk-up
├── cloud/
...
│   ├── status.ts               # MODIFIED: show active project from .valis.json
│   ├── switch.ts               # NEW: valis switch --project <name>
