# Quickstart: Teamind MVP (Developer Setup)
...
cd packages/cli
pnpm build
pnpm link --global   # makes `teamind` available globally
```
...
# Export
teamind export --json --output decisions.json
```

## 6. Test with MCP Inspector

```bash
npx @modelcontextprotocol/inspector teamind serve
```

This opens a browser UI to test `teamind_store`, `teamind_search`,
and `teamind_context` interactively.
